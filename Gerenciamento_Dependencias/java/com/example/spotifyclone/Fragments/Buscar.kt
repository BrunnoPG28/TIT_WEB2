...
class Buscar : Fragment() {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.fragment_burcar, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        secaoAdapter = SecaoAdapter(dados())
        rcl_view_sessao.adapter = secaoAdapter
        rcl_view_sessao.layoutManager = GridLayoutManager(context, 2)
    ...
    private inner class SecaoAdapter(private val secoes : MutableList<Secao>): RecyclerView.Adapter<SecaoHolder>() {
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SecaoHolder {
            val view = layoutInflater.inflate(R.layout.secao_item, parent, false)
    ...
    private inner class SecaoHolder(itemView: View) : RecyclerView.ViewHolder(itemView){
        fun bind(secao : Secao){
            itemView.img_sessao.setImageResource(secao.secao)
            itemView.txt_sessao.text = secao.nomeSecao
        }
